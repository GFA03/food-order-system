...
class AiServiceTest {
    ...
    void setUp() {
        ...
        aiService = new AiService(llmService, restaurantClient);
    ...
    void suggest_usesLlmFilter_andRanksMatchingCatalogue() {
        ...
        when(llmService.extractFilter(anyString())).thenReturn(filter);
        ...
        assertThat(result)
                .anySatisfy(s -> {
                    ...
                    assertThat(s.id()).isEqualTo("r2"); // deep-linked to its restaurant
                });
    ...
    void keywordFallback_searchesCatalogueWithoutLlm() {
        ...
        // The LLM is never consulted on the fallback path.
