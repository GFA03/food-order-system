      ...
      <div className="flex flex-col items-end gap-2 shrink-0">
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-full border border-gray-300 text-gray-600 hover:border-orange-400 hover:text-orange-500 transition-colors"
...
export default function RestaurantDetailPage() {
  ...
  function handleAddToCart(item: MenuItem, quantity: number) {
    console.log('Add to cart:', item.name, 'x', quantity);
  }
  ...
  return (
    <div>
      <Link to="/restaurants" className="text-sm text-orange-500 hover:underline mb-4 inline-block">
        ← Back to Restaurants
      </Link>
          ...
          {restaurant.cuisineTags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-3">
              {restaurant.cuisineTags.map((tag) => (
                <span key={tag.id} className="px-2 py-0.5 bg-orange-50 text-orange-600 text-xs font-medium rounded-full">
                  {tag.name}
                </span>
              ))}
            </div>
